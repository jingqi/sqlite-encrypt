...
#if !defined(SQLITE_OMIT_DISKIO) && defined(SQLITE_HAS_CODEC) && SQLITE_HAS_CODEC
...
static bool handle_error(void *codec)
{
    if (!codec)
        return 0;

    const char *error = codec_get_and_reset_error(codec);
    if (error)
    {
...
// 加密/解密
static void* sqlite3Codec(void *codec, void *data, Pgno page_num, int mode)
{
...
// 报告页大小
static void sqlite3CodecSizeChange(void *codec, int page_size, int reserve)
{
    codec_set_page_size(codec, page_size);
}

// 销毁加密器
static void sqlite3PagerFreeCodec(void *codec)
{
    if (codec)
        codec_delete(codec);
}
...
int sqlite3CodecAttach(sqlite3 *db, int n_db, const void *z_key, int n_key)
{
    ...
    if (NULL == z_key || n_key <= 0)
    {
        ...
        if (0 != n_db && n_key < 0)
        {
            ...
            if (main_codec != NULL)
            {
                codec = codec_new_from_other(main_codec, db);
    ...
    else
    {
        ...
        codec = codec_new(db);
        codec_generate_write_key(codec, (const char*) z_key, n_key);
        codec_set_read_is_write(codec);
        sqlite3PagerSetCodec(sqlite3BtreePager(db->aDb[n_db].pBt),
                             sqlite3Codec,
                             sqlite3CodecSizeChange,
                             sqlite3PagerFreeCodec,
...
SQLITE_API int sqlite3_key_v2(sqlite3 *db, const char *z_db_name, const void *z_key, int n_key)
{
    // We don't use z_db_name (though maybe we could...). Pass-through to the old sqlite_key
    return sqlite3_key(db, z_key, n_key);
}

SQLITE_API int sqlite3_rekey_v2(sqlite3 *db, const char *z_db_name, const void *z_key, int n_key)
{
    // We don't use z_db_name (though maybe we could...). Pass-through to the old sqlite_rekey
    return sqlite3_rekey(db, z_key, n_key);
}

SQLITE_API int sqlite3_key(sqlite3 *db, const void *z_key, int n_key)
{
    // The key is only set for the main database, not the temp database
    return sqlite3CodecAttach(db, 0, z_key, n_key);
}

SQLITE_API int sqlite3_rekey(sqlite3 *db, const void *z_key, int n_key)
{
